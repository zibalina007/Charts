...
<center>@include('charts::_partials.container.div')</center>
...
$(function() {
    ...
    setInterval(function() {
        $.getJSON( "{{ $model->url }}", function( jdata ) {
            ...
            {{ $model->id }}.animate(v, {step: function(state, circle, attachment) {circle.setText(jdata["{{ $model->value_name }}"].toString())}})
        })
    }, {{ $model->interval }})
});
</script>
